...
description: 'Reference for all environment variables used in FocusMail'
---

Comprehensive reference for all environment variables relevant to self-hosting FocusMail.
...
| `RESEND_FROM_EMAIL` | No | From email address | `FocusMail <updates@transactional.getinboxzero.com>` |
...
| `NEXT_PUBLIC_BRAND_NAME` | No | Brand name used in UI text and metadata | `FocusMail` |
| `NEXT_PUBLIC_BRAND_LOGO_URL` | No | Custom logo URL or public asset path (for example `/images/brand-logo.svg`) | Built-in FocusMail logo |
